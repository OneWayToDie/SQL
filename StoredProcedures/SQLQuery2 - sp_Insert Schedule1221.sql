...
BEGIN
	...
	DECLARE @lesson_number		AS TINYINT  = (SELECT COUNT(lesson_id)  FROM Schedule	 WHERE discipline=@discipline AND [group] = @group);--------Нужно учитывать проедённые занятия по выставляемой дисциплине
