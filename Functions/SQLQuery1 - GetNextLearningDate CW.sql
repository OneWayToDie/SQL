--SQLQuery1 - GetNextLearningDate CW.sql
USE PV_521_Import;
SET DATEFIRST 1;
GO

CREATE OR ALTER FUNCTION GetNextLearningDate_CW(@group_name AS NCHAR(10), @date AS DATE = N'1900-01-01')RETURNS DATE
AS
BEGIN
	...
	DECLARE @next_day	AS SMALLINT	=	dbo.GetNextLearningDay_CW(@group_name, @date);
...
--BEGIN
--    SET @next_date = dbo.GetNextLearningDate_CW(@group_name, @next_date);
--END
RETURN
IIF(NOT EXISTS (SELECT holiday FROM DaysOFF WHERE [date] = @next_date), @next_date, dbo.GetNextLearningDate_CW(@group_name, @next_date));
